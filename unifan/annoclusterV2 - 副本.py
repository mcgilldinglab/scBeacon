...
class AnnoCluster(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        # get encoding
        z_e1, _ = self.encoder(x1)

        # get the index of embedding closed to the encoding
        k1, z_dist1, dist_prob1 = self._get_clusters(z_e1)

        # get embeddings (discrete representations)
        z_q1 = self._get_embeddings(k1)

        # get encoding
        z_e2, _ = self.encoder(x2)

        # get the index of embedding closed to the encoding
        k2, z_dist2, dist_prob2 = self._get_clusters(z_e2)

        # get embeddings (discrete representations)
        z_q2 = self._get_embeddings(k2)
