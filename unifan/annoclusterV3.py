...
from layers import weightedZINBLoss, MeanAct, DispAct
...
class AnnoCluster(nn.Module):
    ...
    def __init__(self, input_dim: int = 10000, z_dim: int = 32, gene_set_dim: int = 335,
        ...
        if self.decoding_network == 'gaussian':
            ...
            self.decoder_q = Decoder_ZINB(z_dim, input_dim, num_layers=num_layers_decoder, hidden_dim=emission_dim)
        ...
        self.zinb_loss = weightedZINBLoss().cuda()
    ...
    def forward(self, x):
        ...
        q_mean, q_disp, q_pi= self.decoder_e(z_q)
        return e_mean, e_disp, e_pi, q_mean, q_disp, q_pi, z_e, z_q, k, z_dist, dist_prob
    ...
    def loss(self, x_raw_tensor, e_mean, e_disp, e_pi, q_mean, q_disp, q_pi, dispersion):
        l_e = self.zinb_loss(x=x_raw_tensor, mean=e_mean, disp=e_disp, pi=e_pi, dispersion=dispersion)
        l_q = self.zinb_loss(x=x_raw_tensor, mean=q_mean, disp=q_disp, pi=q_pi, dispersion=dispersion)
        l = l_e + l_q * self.zeta
        return l
