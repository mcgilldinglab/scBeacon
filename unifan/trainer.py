...
class Trainer(nn.Module):
    ...
    def train_epoch_anno_ZINB(self,  **kwargs):
        ...
        for batch_idx, (X_batch, clusters_true, subjects) in enumerate(tqdm(self.dataloader_train)):
            loss, _, _,  loss_e, loss_q = self.process_minibatch_anno_ZINB(X_batch, subject=subjects, **kwargs)
            l = l + loss
            l_e = l_e + loss_e
            l_q = l_q + loss_q
        l = l / batch_idx
    ...
    def process_minibatch_anno_ZINB(self, X_batch, subject, weight_decay: float = 0):
        ...
        if X_batch_1.shape[0]>0 and X_batch_2.shape[0]>0:
            e_mean_1, e_disp_1, e_pi_1, q_mean_1, q_disp_1, q_pi_1, z_e_1, z_q_1, k_1, _, _ = self.model(X_batch_1, 0)
            e_mean_2, e_disp_2, e_pi_2, q_mean_2, q_disp_2, q_pi_2, z_e_2, z_q_2, k_2, _, _ = self.model(X_batch_2, 1)
            ...
            e_mean, e_disp, e_pi, q_mean, q_disp, q_pi, z_e, z_q, k, z_dist, dist_prob = self.model(X_batch_2, 1)
        ...
        return l.detach().cpu().item(), k.detach().cpu().numpy(), \
               z_e.detach().cpu().numpy(), l_e.detach().cpu().item(), l_q.detach().cpu().item()
    ...
    def evaluate_anno_ZINB(self, **kwargs):
        ...
        for batch_idx, (X_batch, clusters_true, subject) in enumerate(tqdm(self.dataloader_train)):
            ...
            l, l_e, l_q = self.model.loss(X_batch, e_mean, e_disp, e_pi, q_mean, q_disp, q_pi).detach().cpu().item()
    ...
    def infer_anno_ZINB(self, **kwargs):
        ...
        with torch.no_grad():
            ...
            for batch_idx, (X_batch, clusters_true, subject) in enumerate(tqdm(self.dataloader_all)):
                ...
                _, k, z_e, _, _ = self.process_minibatch_anno_ZINB(X_batch, subject = subject, **kwargs)
                k_list.append(k)
                z_e_list.append(z_e)
