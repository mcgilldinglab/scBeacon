...
class Decoder_ZINB(nn.Module):
    ...
    def __init__(self, input_dim: int, output_dim: int, num_layers: int = 1, hidden_dim: int = 128, 
        ...
        self._dec_mean = nn.Sequential(nn.Linear(hidden_dim, output_dim), MeanAct())
        self._dec_disp = nn.Sequential(nn.Linear(hidden_dim, output_dim), DispAct())
        self._dec_pi = nn.Sequential(nn.Linear(hidden_dim, output_dim), nn.Sigmoid())
    ...
    def forward(self, x: torch.Tensor):
        ...
        p = self.decoder(x)
        _mean = self._dec_mean(p)
        _disp = self._dec_disp(p)
        _pi = self._dec_pi(p)
